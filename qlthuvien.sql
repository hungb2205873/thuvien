...
--
-- Host: 127.0.0.1
-- Generation Time: Nov 15, 2025 at 10:31 AM
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `CapNhatDocGia` (IN `p_ma_doc_gia` INT, IN `p_ten_doc_gia` VARCHAR(255), IN `p_ngay_sinh` DATE, IN `p_so_dien_thoai` VARCHAR(15), IN `p_email` VARCHAR(255), IN `p_mat_khau` VARCHAR(255))   BEGIN
    UPDATE doc_gia 
    SET 
        ten_doc_gia = p_ten_doc_gia, 
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `CheckPenalty` (IN `p_ma_phieu_muon` INT, OUT `p_has_penalty` BOOLEAN)   BEGIN
    DECLARE v_tien_phat DECIMAL(10,2); -- biến tạm lưu tiền phạt
...
CREATE DEFINER=`root`@`localhost` FUNCTION `total_borrows` (`period_type` VARCHAR(10)) RETURNS INT(11) DETERMINISTIC BEGIN
    ...
    IF period_type = 'today' THEN
        ...
        FROM phieu_muon as pm join chi_tiet_phieu_muon as ctpm 
		on ctpm.ma_phieu_muon = pm.ma_phieu_muon 
        WHERE DATE(ngay_muon) = CURDATE();
    
    ELSEIF period_type = 'this_week' THEN
		SELECT COUNT(*) INTO total 
        FROM phieu_muon as pm join chi_tiet_phieu_muon as ctpm 
		on ctpm.ma_phieu_muon = pm.ma_phieu_muon  
        WHERE  YEARWEEK(pm.ngay_muon, 1) = YEARWEEK(CURDATE(), 1);
    
    ELSEIF period_type = 'this_month' THEN
        SELECT COUNT(*) INTO total FROM phieu_muon as pm 
        join chi_tiet_phieu_muon as ctpm 
		on ctpm.ma_phieu_muon = pm.ma_phieu_muon 
...
(1, 1, 1, 'Cho Tôi Xin Một Vé Đi Tuổi Thơ', '2008', 'NXB Trẻ', 18, '2025-10-12'),
(2, 2, 2, 'Harry Potter và Hòn Đá Phù Thủy', '1997', 'Bloomsbury', 15, '2025-10-12'),
(3, 3, 3, 'Sapiens: Lược Sử Loài Người', '2011', 'HarperCollins', 9, '2025-10-12'),
(4, 4, 4, 'Đắc Nhân Tâm', '1936', 'Simon & Schuster', 20, '2025-10-12'),
(5, 5, 5, 'Doraemon Tập 1', '1970', 'Shogakukan', 23, '2025-10-12'),
(6, 6, 6, 'Thám Tử Lừng Danh Conan Tập 1', '1994', 'Shogakukan', 30, '2025-10-12'),
(7, 7, 7, 'Cha Giàu Cha Nghèo', '1997', 'Plata Publishing', 18, '2025-10-12'),
(8, 8, 8, 'Giáo Trình Cấu Trúc Dữ Liệu và Giải Thuật', '2010', 'NXB Giáo Dục', 10, '2025-10-12'),
(9, 9, 9, 'Project Hail Mary', '2021', 'Ballantine Books', 7, '2025-10-12'),
(10, 10, 10, 'Tomorrow, and Tomorrow, and Tomorrow', '2022', 'Knopf', 10, '2025-10-12'),
(11, 11, 1, 'Nhà Giả Kim', '1988', 'HarperTorch', 20, '2025-10-12'),
(12, 12, 10, 'Norwegian Wood', '1987', 'Kodansha', 13, '2025-10-12'),
(13, 13, 6, 'Mật Mã Da Vinci', '2003', 'Doubleday', 16, '2025-10-12'),
(14, 14, 9, '1984', '1949', 'Secker & Warburg', 25, '2025-10-12'),
(15, 15, 1, 'Phía Tây Không Có Gì Lạ', '1929', 'Propyläen Verlag', 12, '2025-10-12'),
(16, 16, 3, 'Lược Sử Thời Gian', '1988', 'Bantam Books', 20, '2025-10-12'),
(17, 17, 10, 'Hoàng Tử Bé', '1943', 'Reynal & Hitchcock', 26, '2025-10-12'),
(18, 18, 4, 'Think and Grow Rich', '1937', 'The Ralston Society', 17, '2025-10-12'),
(19, 19, 4, 'Outliers: The Story of Success', '2008', 'Little, Brown and Company', 14, '2025-10-12'),
(20, 20, 4, 'Atomic Habits', '2018', 'Avery', 21, '2025-10-12'),
(49, 21, 4, 'Nguyễn Vũ Nguyên', '2004', 'Mẹ', 20, '2025-10-14');
...
--
-- Indexes for table `chi_tiet_phieu_muon`
--
...
--
-- AUTO_INCREMENT for table `chi_tiet_phieu_muon`
--
ALTER TABLE `chi_tiet_phieu_muon`
...
--
ALTER TABLE `doc_gia`
  MODIFY `ma_doc_gia` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=16;
...
--
-- Constraints for table `chi_tiet_phieu_muon`
--
ALTER TABLE `chi_tiet_phieu_muon`
...
--
-- Constraints for table `phieu_tra`
--
ALTER TABLE `phieu_tra`
